...
// stand-in: it turns a selected file into a base64 "data URL" (the whole
// image re-encoded as text) and that text is sent as the "url" field.
// It works today with zero backend changes, but it is not how this
// should work in production — encoding a photo as text bloats it by
// ~33% and stores it directly in the database.
//
...
// with a call to that endpoint and return the hosted URL it responds
// with. That is the ONLY function that needs to change — every caller
// (SubmitTaskModal.jsx) already just does
// `const url = await prepareImageForUpload(file)` and has no idea
// whether "url" is base64 text or a real hosted link. That's the whole
// point of keeping this conversion inside the service layer instead of
// in the component: the component is not coupled to Base64 at all.
...
// be uploaded") rather than HOW it currently works ("convert to base64"),
// so that when the real implementation changes (see TODO(real-upload)
// above), nothing that calls this function needs to change its name or
// its assumptions — only this function's body.
export function prepareImageForUpload(file) {
  // Current (temporary) implementation: base64-encode the file locally,
  // in the browser, and use that as the "url". No network request to any
  // upload service happens here yet.
