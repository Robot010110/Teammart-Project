  ...
  return (
    <div className="px-4 sm:px-6 py-6 max-w-4xl mx-auto animate-fade-up">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h1 className="font-display text-xl font-bold text-white">{t("rm.markets")}</h1>
          <p className="mt-1 text-sm text-[#9AA1B4]">
            {loading ? t("emp.loading") : t("rm.marketsCompanyWide", { count: markets?.length ?? 0 })}
          </p>
        </div>
        <div className="relative w-full sm:w-64">
          ...
          <input
            ...
            placeholder={t("admin.searchMarketsSupervisorsZones")}
      ...
      <div className="mt-5">
        {loading ? (
          ...
          <div className="rounded-2xl p-10 bg-[#171C2E]/80 border border-white/[0.06] text-center text-sm text-[#8B93A8]">
            {query ? t("rm.noMarketsMatchYourSearch") : t("admin.noMarketsExistYet")}
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
