...
function minutesAgo(iso, t) {
  const mins = Math.floor((Date.now() - new Date(iso).getTime()) / 60000);
  if (mins < 1) return t("sup.justNow");
  if (mins < 60) return t("common.minutesAgo", { count: mins });
  return t("common.hoursAgo", { count: Math.floor(mins / 60) });
}

const STATUS_STYLE = {
  COMPLETED: { label: "status.completed", icon: CheckCircle2, tone: "text-emerald-400 bg-emerald-500/10 ring-emerald-500/20" },
  PENDING: { label: "status.pending", icon: Clock3, tone: "text-[#9AA1B4] bg-white/[0.06] ring-white/10" },
  PENDING_REMINDER: { label: "rm.reminderSent", icon: Bell, tone: "text-amber-400 bg-amber-500/10 ring-amber-500/20" },
  NOT_COMPLETED: { label: "rm.notCompleted", icon: AlertTriangle, tone: "text-red-400 bg-red-500/10 ring-red-500/20" },
};
    ...
    <div className="rounded-2xl p-4 bg-[#171C2E]/80 border border-white/[0.06]">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          ...
          <p className="text-xs text-[#8B93A8] mt-0.5">{t("rm.shiftsReportedOf", { done: market.completedCount, total: market.totalShifts })}</p>
          {market.lastReminderAt && (
            <p className="text-[11px] text-[#4C5266] mt-1">{t("rm.reminderSentAgo", { when: minutesAgo(market.lastReminderAt, t) })}</p>
          )}
        </div>
      ...
      {market.status !== "COMPLETED" && (
        ...
        >
          ...
          {reminding ? t("emp.sending") : t("rm.sendReminder")}
        </button>
      )}
    </div>
  );
...
export default function RmCardSalesZoneStatusPage({ onBack }) {
  ...
  async function handleRemind(marketId) {
    ...
    try {
      ...
      setRemindError(err instanceof ApiError ? err.message : t("rm.couldNotSendTheReminderPlease"));
    } finally {
      setRemindingId(null);
    }
  }
  ...
  return (
    <div className="px-4 sm:px-6 py-6 max-w-2xl mx-auto animate-fade-up pb-10">
      <Breadcrumb items={[{ label: t("rm.marketActivities"), onClick: onBack }, { label: t("rm.cardSales") }]} />

      <div className="mt-4 flex items-center gap-2">
        <span className="grid place-items-center h-9 w-9 rounded-xl bg-emerald-500/10 text-emerald-400">
          <CreditCard size={17} />
        </span>
        <h1 className="text-xl font-bold text-white">{t("rm.cardSalesToday")}</h1>
      </div>
      ...
      <div className="mt-4 space-y-2.5">
        {loading ? (
          ...
          <div className="rounded-2xl p-10 bg-[#171C2E]/80 border border-white/[0.06] text-center text-sm text-[#8B93A8]">
            {t("rm.noMarketsAreAssignedToYou")}
          </div>
        ) : (
          data.markets.map((m) => (
