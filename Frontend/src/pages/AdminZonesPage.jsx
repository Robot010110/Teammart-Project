  ...
  return (
    <div className="max-w-6xl mx-auto animate-fade-up">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between mb-5">
        <div>
          <h1 className="font-display text-xl md:text-[26px] font-bold text-white">{t("admin.zonesMarkets")}</h1>
          <p className="mt-1 text-sm text-[#9AA1B4]">{t("admin.manageAllZonesAndMarketsAcross")}</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            <input
              ...
              placeholder={t("admin.searchZones")}
              className="w-56 rounded-xl bg-gradient-to-b from-[#131C31]/90 to-[#0F1728]/90 border border-white/[0.09] ps-10 pe-3 py-2.5 text-sm text-white placeholder:text-[#5C6479] outline-none transition-all duration-150 focus:border-[#F47A20]/55 focus:shadow-[0_0_0_3px_rgba(244,122,32,0.15)]"
            />
          </div>
          <button
            type="button"
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
        <AdminKpiCard icon={Layers} tone="blue" value={loading ? undefined : kpi.totalZones} label={t("admin.totalZones")} loading={loading} />
        <AdminKpiCard icon={Store} tone="green" value={loading ? undefined : kpi.totalMarkets} label={t("admin.totalMarkets")} loading={loading} />
        <AdminKpiCard icon={Users} tone="purple" value={loading ? undefined : kpi.totalEmployees} label={t("rm.totalEmployees")} loading={loading} />
        <AdminKpiCard icon={Gauge} tone="amber" value={loading ? undefined : (kpi.activePct === null ? "—" : `${kpi.activePct}%`)} label={t("admin.activeMarkets")} loading={loading} />
      </div>
          ...
          <div className="space-y-2.5">
            {filteredZones.map((zone, i) => {
              ...
              return (
                ...
                >
                  <span className="pointer-events-none absolute -start-10 -top-10 h-32 w-32 rounded-full bg-[#F47A20]/[0.08] blur-3xl" aria-hidden="true" />
                  <button
                    type="button"
                    onClick={() => navigate(`/admin/zones/${raw}/markets`)}
                    className="relative flex flex-1 min-w-0 items-center gap-3.5 text-start"
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="font-display text-[15px] font-bold text-white">{t("rm.zoneNumbered", { number: zone.number })}</p>
                      <p className="text-[12.5px] text-[#8B93A8] mt-0.5">
                        {t("rm.zoneSummaryLine", { manager: zone.manager, markets: zone.marketsCount, employees: zone.employeesCount })}
                      </p>
                    </div>
...
function CreateZoneModal({ onClose, onCreated }) {
  ...
  async function handleCreate() {
    ...
    if (!Number.isInteger(n) || n <= 0) {
      setError(t("admin.enterAPositiveWholeNumber"));
      return;
    }
    setSaving(true);
    setError(null);
    try {
      ...
      setError(err instanceof ApiError ? err.message : t("admin.couldNotCreateTheZone"));
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <Modal open onClose={onClose} title={t("admin.newZone")}>
      <label className="block text-xs uppercase tracking-wide text-[#8B93A8] mb-2">{t("admin.zoneNumber")}</label>
      <input
        type="number"
        min="1"
        value={number}
        onChange={(e) => setNumber(e.target.value)}
        placeholder={t("admin.eG4")}
        autoFocus
...
function AssignManagerModal({ zone, onClose, onAssigned }) {
  ...
  async function handleAssign(managerId) {
    ...
    try {
      ...
      setError(err instanceof ApiError ? err.message : t("admin.couldNotAssignTheManager"));
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <Modal open onClose={onClose} title={t("rm.zoneManagerTitle", { number: zone.number })}>
      {managersLoading ? (
        ...
        <div className="space-y-2">
          <button
            ...
            className="w-full text-start rounded-lg px-3.5 py-3 text-sm text-[#9AA1B4] bg-white/[0.03] hover:bg-white/[0.06] border border-white/[0.06] transition-colors duration-150"
          >
            {t("rm.unassigned")}
          </button>
          {managers.length === 0 ? (
            <p className="text-xs text-[#6B7284] px-1 py-2">
              {t("admin.noRegionalManagerAccountsExistYet")}
            </p>
          ) : (
            managers.map((m) => (
              <button
                ...
                className="w-full text-start rounded-lg px-3.5 py-3 text-sm text-white bg-white/[0.03] hover:bg-white/[0.06] border border-white/[0.06] transition-colors duration-150"
              >
