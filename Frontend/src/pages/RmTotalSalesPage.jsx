...
function ReportCard({ report, onReview, reviewing, onDelete, deleting }) {
  ...
  async function handleReject() {
    if (!reason.trim()) {
      setError(t("rm.aReasonIsRequiredToReject"));
      return;
    }
    setError(null);
      ...
      <div className="flex items-center gap-4">
        {report.photoUrl && (
          <a href={report.photoUrl} target="_blank" rel="noreferrer" className="shrink-0">
            <AuthenticatedImage src={report.photoUrl} alt={t("emp.evidence")} className="h-16 w-16 rounded-xl object-cover ring-1 ring-white/10" />
          </a>
        )}
        <div className="min-w-0 flex-1">
          ...
          {report.status !== "PENDING" && report.reviewedBy && (
            <p className="mt-1 text-[11px] text-[#4C5266]">{t("rm.reviewedByName", { name: report.reviewedBy.name })}</p>
          )}
        </div>
      </div>
          ...
          {rejecting ? (
            <div className="space-y-2">
              <input
                ...
                placeholder={t("rm.reasonForRejecting")}
...
export default function RmTotalSalesPage({ marketId, marketName, onBack }) {
  ...
  async function handleReview(id, status, rejectionReason) {
    ...
    try {
      ...
      setReviewError(err instanceof ApiError ? err.message : t("sup.couldNotSubmitThisReviewPlease"));
    } finally {
      setReviewingId(null);
    }
  }
  ...
  async function handleDelete(id) {
    ...
    try {
      ...
      setReviewError(err instanceof ApiError ? err.message : t("rm.couldNotDeleteThisReportPlease"));
    } finally {
      setDeletingId(null);
    }
  }
  ...
  return (
    <div className="px-6 md:px-10 py-8 max-w-4xl mx-auto animate-fade-up">
      <Breadcrumb items={[{ label: t("rm.markets"), onClick: onBack }, { label: marketName ?? t("sup.market"), onClick: onBack }, { label: t("rm.totalSales") }]} />

      <div className="mt-4 flex items-center justify-between gap-4">
        <h1 className="font-display text-2xl font-bold text-white">{t("rm.totalSales")}</h1>
        <input
          type="date"
          value={dateFilter}
      ...
      <div className="mt-6">
        {loading ? (
          ...
          <div className="rounded-2xl p-10 bg-[#171C2E]/80 border border-white/[0.06] text-center text-sm text-[#8B93A8]">
            {dateFilter ? t("rm.noTotalSalesReportForThis") : t("rm.noTotalSalesReportsSubmittedYet")}
          </div>
        ) : (
          <>
            {pending.length > 0 && (
              <section className="mb-6">
                <h2 className="mb-3 text-xs font-semibold uppercase tracking-wider text-[#8B93A8]">{t("emp.pendingReview")}</h2>
            ...
            {history.length > 0 && (
              <section>
                <h2 className="mb-3 text-xs font-semibold uppercase tracking-wider text-[#8B93A8]">{t("emp.history")}</h2>
                <div className="space-y-3">
                  {history.map((r) => (
