  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      searchProducts({ search: searchTerm.trim() })
        .then(setProducts)
        .catch((err) => setError(err instanceof ApiError ? err.message : t("emp.couldNotSearchProducts")))
    ...
    <div className="space-y-3">
      {evidencePhotoUrl && (
        <div className="flex items-center gap-2 rounded-lg p-2 bg-white/[0.04]">
          ...
          <span className="text-xs text-[#9AA1B4]">{t("emp.photoAttachedAsEvidence")}</span>
        </div>
      )}
      <div className="relative">
        ...
        <input
          ...
          placeholder={t("emp.searchProductByName")}
          className="w-full rounded-lg bg-white/[0.04] border border-white/[0.06] ps-9 pe-3 py-3 text-base sm:text-sm text-white placeholder:text-[#4C5266] outline-none focus:border-[#F47A20]/50"
        />
      </div>
      {error && <p className="text-xs text-red-400">{error}</p>}
      <div className="space-y-2 max-h-[260px] overflow-y-auto">
        {searching && <p className="text-xs text-[#4C5266] text-center py-4">{t("emp.searching")}</p>}
        {!searching && searchTerm.trim() && products.length === 0 && !error && (
          <p className="text-xs text-[#4C5266] text-center py-4">{t("emp.noProductsFound")}</p>
        )}
        {!searching &&
          products.map((product) => (
            <button
              ...
              className="w-full text-start rounded-lg p-3 bg-[#1A1F33]/70 border border-white/[0.06] hover:border-[#F47A20]/35 active:bg-[#1F2436] transition-colors duration-150"
            >
