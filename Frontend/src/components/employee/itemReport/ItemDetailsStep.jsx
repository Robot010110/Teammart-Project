...
const CONDITIONS = [
  { value: "EXPIRED", label: "emp.expired" },
  { value: "WASTED", label: "emp.wasted" },
];
...
export default function ItemDetailsStep({
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        <label className="block text-xs uppercase tracking-wide text-[#8B93A8] mb-1.5">{t("emp.condition")}</label>
      ...
      <div>
        <label className="block text-xs uppercase tracking-wide text-[#8B93A8] mb-1.5">{t("emp.quantity")}</label>
        <input
          type="number"
          inputMode="numeric"
          min="1"
          ...
          placeholder={t("emp.numberOfItems")}
