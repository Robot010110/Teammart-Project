...
const TYPES = [
  { value: "MONTHLY_OFF", label: "emp.monthlyOffDay" },
  { value: "PERSONAL_LEAVE", label: "emp.personalLeave" },
  { value: "EARNED_DAY_OFF", label: "emp.earnedDayOff" },
];
...
export default function LeaveRequestFlow({ open, onClose, onSaved }) {
  ...
  const handleSubmit = async () => {
    if (type === "PERSONAL_LEAVE" && !reason.trim()) {
      ...
      setError(t("emp.aReasonIsRequiredForPersonal"));
      return;
    }
    ...
    try {
      ...
      onSaved(request, t("emp.leaveRequestSubmitted"));
      handleClose();
    } catch (err) {
      setError(err instanceof ApiError ? err.message : t("emp.couldNotSubmitThisRequestPlease"));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <Modal open={open} onClose={handleClose} title={t("emp.requestOffDayLeave")}>
      <div className="space-y-4">
        ...
        <div>
          <label className="block text-xs uppercase tracking-wide text-[#8B93A8] mb-1.5">{t("emp.requestType")}</label>
            ...
            {TYPES.map((opt) => (
              <button
                ...
                className={`flex-1 rounded-lg py-3 text-sm font-medium transition-colors duration-150 ${
                  type === opt.value ? "bg-[#F47A20] text-white" : "bg-white/[0.05] text-[#9AA1B4] active:bg-white/[0.09]"
                }`}
              >
                {t(opt.label)}
              </button>
        ...
        {type === "EARNED_DAY_OFF" && (
          <div className="rounded-lg p-3.5 bg-white/[0.04] border border-white/[0.06] space-y-1.5">
            ...
            <div className="flex items-center justify-between text-sm">
              <span className="text-[#8B93A8]">{t("emp.required")}</span>
              <span className="font-semibold text-white">{EXTRA_HOURS_PER_DAY_OFF}h</span>
            </div>
            {!balanceLoading && balance != null && !hasEnoughBalance && (
              <p className="text-xs text-amber-400 pt-1">{t("emp.notEnoughExtraHoursYetFor")}</p>
            )}
          </div>
        )}
        ...
        {type === "PERSONAL_LEAVE" && (
          <div>
            ...
            <textarea
              ...
              rows={3}
              placeholder={t("emp.eGIllnessFamilyEmergencyPersonal")}
        ...
        >
          ...
          {submitting ? t("emp.submitting") : t("emp.submitRequest")}
        </button>
      </div>
    </Modal>
  );
