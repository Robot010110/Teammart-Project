...
function Column({ icon: Icon, value, unit, label, tone, trendDir }) {
  return (
    <div className="flex-1 min-w-0 flex items-center gap-2 px-1.5 first:ps-0 last:pe-0">
      <span
...
export default function HighlightsCard({ weekly, activities }) {
  ...
  if (streak > 0) {
    columns.push({
      ...
      icon: Flame,
      value: streak,
      unit: streak === 1 ? "Day" : t("emp.days"),
      label: t("emp.consistencyStreak"),
      tone: TONES.emerald,
    });
  }
  ...
  return (
    <section className="relative overflow-hidden rounded-[22px] p-4 bg-[#0D1223]/80 border border-white/[0.07] shadow-[0_10px_40px_-14px_rgba(0,0,0,0.8)]">
      ...
      <div className="absolute inset-0 pointer-events-none" aria-hidden="true">
        <div className="absolute -top-20 -end-10 w-52 h-52 rounded-full bg-[#F47A20]/[0.09] blur-3xl animate-ambient-drift" />
        <div
          className="absolute -bottom-24 -start-12 w-52 h-52 rounded-full bg-violet-500/[0.09] blur-3xl animate-ambient-drift"
      ...
      <h2 className="relative text-[15px] font-bold text-white">{t("emp.yourHighlights")}</h2>
