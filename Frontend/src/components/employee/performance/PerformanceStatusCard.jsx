...
export default function PerformanceStatusCard({ tone, count, onClick }) {
  ...
  return (
    <Wrapper
      {...(onClick ? { type: "button", onClick, "aria-label": `${count} ${t(toneStyle.label)}. ${t("emp.viewDetails")}` } : {})}
      className={`w-full rounded-2xl ps-2.5 pe-1.5 py-2.5 bg-[#12172A]/70 border ${toneStyle.border} flex items-center gap-2 transition-all duration-150 ${
        onClick ? "active:scale-[0.98] hover:bg-[#182034]/80" : ""
      }`}
    >
      <span className={`shrink-0 w-[26px] h-[26px] rounded-full grid place-items-center ${toneStyle.text} ${toneStyle.ringGlow} bg-white/[0.03]`}>
        <toneStyle.Icon size={14} strokeWidth={2.2} />
      </span>
      ...
      <span className="flex-1 min-w-0 text-start text-[12.5px] font-semibold text-white whitespace-nowrap">
        {t(toneStyle.label)}
      </span>

      <span className={`shrink-0 font-display text-[17px] font-bold tabular-nums leading-none ${toneStyle.text}`}>
        <AnimatedNumber value={count} />
      </span>

      {onClick && <ChevronRight size={14} className="shrink-0 text-[#5C6479] rtl-flip" />}
    </Wrapper>
  );
}
