...
export default function QuickReportRow({ icon: Icon, title, subtitle, onClick, badge }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="w-full flex items-center gap-3 rounded-xl p-3.5 bg-[#171C2E]/80 border border-white/[0.06] backdrop-blur-xl hover:border-[#F47A20]/25 transition-colors"
    >
      <span className="w-10 h-10 rounded-lg bg-[#F47A20]/10 flex items-center justify-center text-[#F47A20] shrink-0">
        <Icon size={18} />
      </span>
      ...
      {badge > 0 && (
        <span className="shrink-0 min-w-[20px] h-5 px-1.5 rounded-full bg-[#F47A20] text-white text-[10px] font-bold flex items-center justify-center">
          {badge}
        </span>
      )}
