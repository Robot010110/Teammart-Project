...
export default function DailyActivityCard({ icon: Icon, title, description, onClick }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="card-premium relative shrink-0 w-[210px] h-[172px] rounded-2xl p-4 bg-gradient-to-br from-[#1A1F33] to-[#171C2E] border border-white/[0.06] overflow-hidden text-start snap-start transition-all duration-200 hover:border-[#F47A20]/30 active:scale-[0.98]"
    >
      <Icon size={148} strokeWidth={1} className="absolute -end-8 -bottom-8 text-white/[0.05] pointer-events-none" aria-hidden="true" />

      <span className="relative w-10 h-10 rounded-lg bg-[#F47A20]/15 flex items-center justify-center text-[#F47A20] glow-orange">
        <Icon size={18} />
      </span>
