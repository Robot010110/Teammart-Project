...
function identity(profile, t) {
  if (!profile) return { label: t("settings.employeeId"), value: "—", hint: "" };
  if (profile.kind === "staff") return { label: t("settings.userId"), value: profile.loginId ?? profile.email, hint: t("settings.userIdHint") };
  if (profile.role === "CASHIER") return { label: t("settings.userId"), value: profile.username, hint: t("settings.userIdHint") };
  return { label: t("settings.employeeId"), value: profile.employeeCode, hint: t("settings.employeeIdHint") };
}
    ...
    <div>
      ...
      <div className="relative">
        <input
          ...
          className="w-full rounded-xl border border-white/[0.08] bg-white/[0.04] py-3 ps-3.5 pe-11 text-[15px] text-white outline-none transition-colors focus:border-[#F47A20]/60"
        />
        ...
        <button
          ...
          onClick={() => setShow((v) => !v)}
          tabIndex={-1}
          aria-label={show ? t("emp.hide") : t("emp.show")}
          className="absolute end-3 top-1/2 -translate-y-1/2 p-1.5 text-[#5C6479] transition-colors hover:text-[#9AA1B4]"
        >
...
// masked real value.
export default function AccountSettings({ profile, loading }) {
  const { t } = useTranslation();
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      setError(err instanceof ApiError ? err.message : t("emp.couldNotChangeYourPasswordPlease"));
    } finally {
      setSaving(false);
    }
  }
    ...
    <div className="space-y-3">
      ...
      <div className="rounded-2xl border border-white/[0.07] bg-[#111A2D]/80 p-4 backdrop-blur-xl">
        <div className="flex items-center gap-3">
          ...
          <p className="text-[12px] text-[#8B93A8]">{t("settings.password")}</p>
        </div>
        <p className="mt-2 select-none font-display text-[20px] font-bold tracking-[0.2em] text-white" aria-label={t("emp.passwordHidden")}>
          ••••••••
        </p>
        <p className="mt-0.5 text-[11.5px] text-[#5C6479]">{t("settings.passwordHidden")}</p>
      </div>
      ...
      {saved && (
        <p className="flex items-center gap-1.5 rounded-xl border border-emerald-500/20 bg-emerald-500/[0.07] px-3.5 py-2.5 text-[12.5px] text-emerald-400">
          <Check size={14} /> {t("settings.passwordChanged")}
        </p>
      )}
      ...
      {!open ? (
        ...
        <form onSubmit={handleSubmit} className="animate-fade-up space-y-3 rounded-2xl border border-white/[0.07] bg-[#111A2D]/80 p-4 backdrop-blur-xl">
          <div className="flex items-center justify-between">
            <p className="text-[14px] font-semibold text-white">{t("settings.changePassword")}</p>
            <button
              type="button"
          ...
          <PasswordInput label={t("settings.currentPassword")} value={currentPassword} onChange={setCurrentPassword} autoFocus />
          <PasswordInput label={t("settings.newPassword")} value={newPassword} onChange={setNewPassword} />
          <PasswordInput label={t("settings.confirmNewPassword")} value={confirmPassword} onChange={setConfirmPassword} />
          ...
          >
            {saving ? <><Loader2 size={16} className="animate-spin" /> {t("emp.saving")}</> : t("settings.changePassword")}
          </button>
        </form>
      )}
      ...
      <div className="flex items-start gap-2.5 rounded-2xl border border-[#7EA6FF]/15 bg-[#7EA6FF]/[0.05] px-3.5 py-3">
        ...
        <p className="text-[11.5px] leading-relaxed text-[#9AA1B4]">{t("settings.passwordNotice")}</p>
      </div>
    </div>
  );
}
