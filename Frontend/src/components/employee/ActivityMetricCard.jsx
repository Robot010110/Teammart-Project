...
// activity, or Compliance with no tasks assigned today). Numeric values
// count up on mount via AnimatedNumber; pre-formatted strings (e.g.
// "4h 35m") render as-is — see that component's own comment.
const TONE = {
  orange: { bg: "bg-[#F47A20]/10", text: "text-[#F47A20]", glow: "glow-orange" },
  emerald: { bg: "bg-emerald-500/10", text: "text-emerald-400", glow: "glow-emerald" },
...
export default function ActivityMetricCard({ icon: Icon, value, label, tone = "orange" }) {
  ...
  return (
    <div className="card-premium flex-1 min-w-[74px] flex flex-col items-center gap-1.5 rounded-xl p-3 bg-[#1A1F33]/70 border border-white/[0.06] transition-transform active:scale-[0.97]">
      <span className={`w-9 h-9 rounded-full flex items-center justify-center ${t.bg} ${t.text} ${t.glow}`}>
        <Icon size={16} />
      </span>
