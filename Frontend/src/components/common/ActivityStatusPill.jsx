...
const STYLE = {
  DRAFT: { icon: FileEdit, labelKey: "status.draft", tone: "bg-white/5 text-[#9AA1B4] ring-white/10" },
  PENDING: { icon: HourglassIcon, labelKey: "status.pending", tone: "bg-amber-500/10 text-amber-400 ring-amber-500/20" },
  APPROVED: { icon: CheckCircle2, labelKey: "status.approved", tone: "bg-emerald-500/10 text-emerald-400 ring-emerald-500/20" },
  REJECTED: { icon: XCircle, labelKey: "status.rejected", tone: "bg-red-500/10 text-red-400 ring-red-500/20" },
};

export default function ActivityStatusPill({ status }) {
  const { t } = useTranslation();
  const { icon: Icon, labelKey, tone } = STYLE[status] || STYLE.PENDING;
  return (
    <span
