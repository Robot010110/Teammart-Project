...
const LABEL_STYLE = {
  DRAFT: { label: "emp.notStarted", tone: "bg-white/5 text-[#9AA1B4]" },
  PENDING: { label: "emp.pendingReview", tone: "bg-amber-500/10 text-amber-400" },
  APPROVED: { label: "status.completed", tone: "bg-emerald-500/10 text-emerald-400" },
  REJECTED: { label: "status.rejected", tone: "bg-red-500/10 text-red-400" },
};
...
function ActivityRow({ activity }) {
  ...
  return (
    <div className="rounded-xl bg-[#1A1F33]/70 border border-white/[0.06] overflow-hidden">
      <button type="button" onClick={() => setExpanded((v) => !v)} className="w-full flex items-center gap-3 p-3.5 text-start">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-xs text-[#8B93A8] mt-0.5">
            {activity.nightShiftTaskDefinition?.name || t("emp.nightShiftTask")} — {activity.images.length} photo{activity.images.length === 1 ? "" : "s"}
          </p>
        </div>
        <span className={`shrink-0 text-[10px] font-semibold rounded-full px-2 py-1 ${style.tone}`}>{t(style.label)}</span>
      ...
      {expanded && (
        <div className="px-3.5 pb-3.5 border-t border-white/[0.06] pt-3">
          <p className="text-[11px] text-[#8B93A8] mb-2.5">
            {activity.status === "DRAFT" ? t("sup.notYetSubmitted") : `Submitted ${timeLabel(activity.updatedAt)}`}
          </p>
          {activity.images.length > 0 ? (
            ...
            <p className="flex items-center gap-1.5 text-xs text-[#4C5266]">
              <Camera size={12} /> {t("sup.noEvidenceUploadedYet")}
            </p>
          )}
        </div>
      )}
...
export default function NightShiftMonitoringSection({ marketId }) {
  ...
  const { data: activities, error, loading, reload } = useAsync(
    ...
    { deps: [marketId], fallbackError: t("sup.couldNotLoadNightShiftActivity") }
  );
  ...
  if (!activities || activities.length === 0) {
    return (
      <div className="rounded-2xl p-6 bg-[#171C2E]/80 border border-white/[0.06] text-center">
        <p className="text-sm text-[#8B93A8]">{t("sup.noNightShiftActivityYetThis")}</p>
      </div>
    );
  }
