...
const SHIFT_CONFIG = {
  MORNING: { label: "emp.morningShift", subtitle: "sup.submitMorningShiftSales", icon: Sunrise, tone: "bg-amber-500/10 text-amber-400" },
  AFTERNOON: { label: "sup.afternoonShift", subtitle: "sup.submitAfternoonShiftSales", icon: Sun, tone: "bg-sky-500/10 text-sky-400" },
  NIGHT: { label: "emp.nightShift", subtitle: "sup.submitNightShiftSales", icon: Moon, tone: "bg-violet-500/10 text-violet-400" },
};
      ...
      {["MORNING", "AFTERNOON", "NIGHT"].map((shift) => {
            ...
            <div key={shift} className="rounded-2xl p-4 bg-[#171C2E]/80 border border-white/[0.06] flex flex-wrap items-center gap-3">
              <div className="flex items-center gap-3 flex-1 min-w-[150px]">
                ...
                <div className="min-w-0">
                  <p className="text-sm font-semibold text-white">{t(cfg.label)}</p>
              ...
              <div className="flex items-center gap-2 ms-auto">
                <span className="shrink-0 flex items-center gap-1 rounded-full px-2.5 py-1 text-[11px] font-semibold text-emerald-400 bg-emerald-500/10">
                  <CheckCircle2 size={12} /> {t("emp.submitted")}
                </span>
                <button
                  type="button"
        ...
        return (
          <button
            ...
            className="w-full flex flex-wrap items-center gap-3 rounded-2xl p-4 text-start bg-[#171C2E]/80 border border-white/[0.06] hover:border-[#F47A20]/25 active:border-[#F47A20]/40 transition-colors"
          >
            <div className="flex items-center gap-3 flex-1 min-w-[150px]">
              <span className={`grid place-items-center h-10 w-10 rounded-xl shrink-0 ${cfg.tone}`}>
                <Icon size={18} />
              ...
              <div className="min-w-0">
                <p className="text-sm font-semibold text-white">{t(cfg.label)}</p>
                <p className="text-xs text-[#8B93A8] mt-0.5">{t(cfg.subtitle)}</p>
              </div>
            </div>
            <div className="flex items-center gap-2 ms-auto">
              <span className="shrink-0 rounded-full px-2.5 py-1 text-[11px] font-semibold text-amber-400 bg-amber-500/10">{t("status.pending")}</span>
