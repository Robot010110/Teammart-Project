...
export default function DepartmentReportReviewModal({ open, section, dept, marketId, onClose, onReviewed }) {
  ...
  async function handleApprove() {
    ...
    try {
      ...
      setError(err instanceof ApiError ? err.message : t("sup.couldNotApproveThisReportPlease"));
    } finally {
      setSubmitting(false);
    }
  }
  ...
  async function handleDecline() {
    if (!reason.trim()) {
      setError(t("sup.aReasonIsRequiredToDecline"));
      return;
    }
    setSubmitting(true);
    setError(null);
    try {
      ...
      setError(err instanceof ApiError ? err.message : t("sup.couldNotDeclineThisReportPlease"));
    } finally {
      setSubmitting(false);
    }
  }
  ...
  return (
    <Modal open={open} onClose={handleClose} title={section ? t("sup.departmentTitle", { department: t(section.labelKey) }) : ""}>
      {!submission || wasDeclined ? (
        <div className="space-y-4">
          {wasDeclined && (
            <div className="rounded-xl p-3.5 bg-red-500/[0.06] border border-red-500/20">
              <p className="flex items-center gap-1.5 text-xs font-semibold text-red-400">
                <XCircle size={13} /> {t("sup.lastSubmissionWasDeclined")}
              </p>
          ...
          <div className="rounded-xl p-4 bg-[#171C2E]/80 border border-white/[0.06] text-center">
            <p className="text-sm text-white font-medium">{t("sup.noReportSubmittedYetToday")}</p>
            <p className="mt-1 text-xs text-[#8B93A8]">
              {dept?.assignedEmployees?.length > 0
                ? t("sup.waitingOnToSubmit", { names: dept.assignedEmployees.map((e) => e.name).join(", ") })
                : t("sup.noEmployeeIsCurrentlyAssignedTo")}
            </p>
          </div>
        </div>
      ) : (
        <div className="space-y-4">
          ...
          {submission.notes && (
            <div>
              <p className="text-xs uppercase tracking-wide text-[#8B93A8] mb-1.5">{t("emp.notes")}</p>
              <p className="text-sm text-[#D5D8E4]">{submission.notes}</p>
            </div>
          )}

          <div>
            <p className="text-xs uppercase tracking-wide text-[#8B93A8] mb-1.5">{t("sup.photos")}</p>
                ...
                {submission.images.map((img) =>
                  img.expired || !img.url ? (
                    <div key={img.id} className="aspect-square rounded-xl bg-white/[0.04] border border-white/[0.06] grid place-items-center gap-1.5 text-[#4C5266]">
                      <ImageOff size={18} />
                      <span className="text-[10px]">{t("sup.photoExpired")}</span>
                    </div>
                  ) : (
                    <div key={img.id} className="aspect-square rounded-xl overflow-hidden border border-white/[0.06]">
                      <AuthenticatedImage src={img.url} alt={t("sup.departmentEvidence")} className="w-full h-full object-cover" />
                    </div>
                  )
                )}
              </div>
            ) : (
              <p className="text-xs text-[#4C5266]">{t("sup.noPhotosAttached")}</p>
            )}
          </div>
            ...
            declining ? (
              <div className="space-y-2.5 pt-1">
                <input
                  ...
                  placeholder={t("sup.reasonForDeclining")}
                ...
                <div className="flex gap-2">
                  ...
                  <button type="button" onClick={handleDecline} disabled={submitting} className="flex-1 flex items-center justify-center gap-1.5 rounded-xl py-2.5 text-sm font-semibold text-white bg-red-600 hover:bg-red-500 disabled:opacity-50 transition-colors">
                    {submitting ? <Loader2 size={14} className="animate-spin" /> : <XCircle size={14} />} {t("sup.confirmDecline")}
                  </button>
                </div>
              </div>
            ) : (
              <div className="flex gap-2 pt-1">
                ...
                <button type="button" onClick={handleApprove} disabled={submitting} className="flex-1 flex items-center justify-center gap-1.5 rounded-xl py-3 text-sm font-semibold text-white bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 transition-colors">
                  {submitting ? <Loader2 size={14} className="animate-spin" /> : <CheckCircle2 size={14} />} {t("sup.approve")}
                </button>
              </div>
            )
          )}
          ...
          {isApproved && (
            <p className="flex items-center justify-center gap-1.5 text-sm font-semibold text-emerald-400 pt-1">
              <CheckCircle2 size={15} /> {t("sup.approvedForToday")}
            </p>
          )}
        </div>
      )}
