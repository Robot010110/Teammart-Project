...
export default function SupervisorAnnouncementsCard({ session, basePath }) {
  ...
  return (
    <button
      ...
      className="w-full text-start rounded-2xl p-4 sm:p-5 border backdrop-blur-xl transition-colors bg-amber-500/[0.06] border-amber-500/20 hover:border-amber-500/35"
    >
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <p className="text-sm font-semibold text-white">{t("sup.zoneAnnouncements")}</p>
          {latest ? (
            <>
              <p className="text-xs text-[#9AA1B4] mt-0.5 line-clamp-2">{latest.body || "(attachment)"}</p>
              <p className="text-[11px] text-[#4C5266] mt-1">
                {(latest.senderUser?.name || latest.senderEmployee?.name) ?? t("sup.unknown")} · {timeAgo(latest.createdAt, t)}
              </p>
            </>
          ) : (
            <p className="text-xs text-[#8B93A8] mt-0.5">{t("sup.noAnnouncementsYet")}</p>
          )}
        </div>
        <ChevronRight size={16} className="text-amber-400/60 shrink-0 mt-1 rtl-flip" />
      </div>
    </button>
  );
}
