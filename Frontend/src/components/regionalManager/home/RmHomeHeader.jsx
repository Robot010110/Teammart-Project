    ...
    <header>
      ...
      <div className="mt-4 flex items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="text-[13px] text-[#8B93A8]">{t(greeting())}</p>
          ...
          >
            ...
            <span className="truncate">{t("roles.regionalManager")}{zoneLabel ? ` · ${zoneLabel}` : ""}</span>
          </button>
        </div>
        <p className="shrink-0 pt-1 text-end text-[11.5px] leading-tight text-[#5C6479]">
          {dayLabel()}
          <br />
          <span className="text-[#8B93A8]">{t("rm.disciplineToday")}</span>
          <br />
          <span className="text-[#8B93A8]">{t("rm.strongerTomorrow")}</span>
        </p>
      </div>
    </header>
  );
